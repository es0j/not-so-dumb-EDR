...
void kernelInjector(char* dll_full_path) {
    ...
    while (TRUE) {
        ...
        if (isPipeConnected) {
            ...
            Inject(target_pid, dll_full_path);
...
int main(int argc,char **argv) {
    ...
    if (argc < 2) {
        printf("Starting in kernel agent mode\n");
        kernelInjector(dll_full_path);
    }
    else
    {
        DWORD pid = atoi(argv[1]);
        Inject(pid, dll_full_path);
    }
