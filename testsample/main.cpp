...
int main(int argc, char* argv[]) {
	...
	// IMAGE_NT_HEADERS
	imageNTHeaders = (PIMAGE_NT_HEADERS)((PTRSIZE)fileData + dosHeader->e_lfanew);
	printf("\n******* NT HEADERS *******\n");
	printf("\t%p\t\tSignature\n", fileData);
	printf("\t%p\t\tSignature\n", imageNTHeaders);
	...
	PTRSIZE sectionLocation = (PTRSIZE)imageNTHeaders + sizeof(DWORD) + (DWORD)(sizeof(IMAGE_FILE_HEADER)) + (PTRSIZE)imageNTHeaders->FileHeader.SizeOfOptionalHeader;
	...
	rawOffset = (PTRSIZE)fileData + importSection->PointerToRawData;
